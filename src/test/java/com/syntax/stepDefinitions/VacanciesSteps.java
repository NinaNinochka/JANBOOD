...
public class VacanciesSteps extends BaseClass {
	...
	public void i_enter_following_values_and_click_save(DataTable Table1) throws Throwable {
		...
		this.JobTitleManager = map.get("JobTitleManager");
	...
	public void i_see_Vacancy_appears() throws Throwable {
		List<WebElement> rows = vac.table.findElements(By.tagName("tr"));
		System.out.println("Number of rows--------------------"+rows.size());
		for (WebElement row : rows) {
			String positionName = row.getText();
			System.out.println("actual text from table -------------------------------"+positionName);
			System.out.println("Expected job title--------------------------"+JobTitleManager);
			if (positionName.contains(JobTitleManager)) {
